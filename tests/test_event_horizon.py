...
class TestStructure:
    ...
    def test_rgb_values_in_range(self):
        viz = EventHorizon()
        for _ in range(5):
            colors = viz.render(_make_frame(rms=0.7, bass=0.4, mids=0.3))
        for r, g, b in colors:
            assert 0 <= r <= 255
            assert 0 <= g <= 255
            assert 0 <= b <= 255
...
class TestSilence:
    def test_silence_is_mostly_dark(self):
        ...
        silent = _make_frame(rms=0.0, dominant_freq=0.0)
...
class TestCollapse:
    def test_beat_triggers_collapse(self):
        ...
        viz.render(_make_frame(is_beat=True))
...
class TestParticles:
    ...
    def test_particles_die_over_time(self):
        ...
        for _ in range(30):
            viz.render(_make_frame(is_beat=False))
...
class TestDeterminism:
    def test_deterministic_output(self):
        ...
        frames = [
            _make_frame(rms=0.5, bass=0.3, mids=0.2, dominant_freq=220.0),
            _make_frame(rms=0.7, bass=0.8, mids=0.1, is_beat=True),
            _make_frame(rms=0.6, bass=0.2, mids=0.4, dominant_freq=880.0),
        ]
    ...
    def test_different_seeds_differ(self):
        ...
        frame = _make_frame(rms=0.5, is_beat=True)
        viz_a = EventHorizon(seed=42)
...
class TestRingStructure:
    def test_ring_structure(self):
        ...
        warm = _make_frame(rms=0.8, bass=0.3, mids=0.2, dominant_freq=440.0)
